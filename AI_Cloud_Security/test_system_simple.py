...
def test_dataset():
    ...
    if headers != expected_headers:
        print(f"[FAIL] Headers mismatch! Expected: {expected_headers}, Got: {headers}")
        return False
    ...
    print(f"[PASS] Dataset OK: {len(rows)} rows with correct headers")
    return True
    ...
    try:
        ...
        for key in required_keys:
            if key not in model_data:
                print(f"[FAIL] Missing key in model: {key}")
                return False
        
        print("[PASS] Model OK: All components present")
        return True
...
def test_prediction():
    ...
    try:
        ...
        print(f"[PASS] Prediction OK: {result['prediction']} (confidence: {result['confidence']:.3f})")
        return True
    except Exception as e:
...
def main():
    ...
    if passed == total:
        ...
        print("[INFO] Run 'python run_app.py' to start the web application")
    else:
